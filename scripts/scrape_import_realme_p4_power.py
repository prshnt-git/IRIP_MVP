#!/usr/bin/env python3
"""Scrape, clean, and import Realme P4 Power 5G reviews into IRIP.

Steps:
  1. Playwright scrape (up to 200 reviews, 30 pages)
...
Run from repo root:
    python scripts/scrape_import_realme_p4_power.py
"""
...
PRODUCT_ID = "realme_p4_power_5g"
PRODUCT_NAME = "Realme P4 Power 5G"
BRAND = "Realme"
FLIPKART_URL = "https://www.flipkart.com/realme-p4-power-5g-transsilver-128-gb/p/itm49784b322aad0"
DB_PATH = BACKEND_DIR / "data" / "irip_mvp.db"
RAW_CSV = BACKEND_DIR / "data" / "realme_p4_power_5g_reviews_raw.csv"
CLEAN_CSV = BACKEND_DIR / "data" / "realme_p4_power_5g_reviews_clean.csv"
RENDER_URL = "https://irip-api.onrender.com/reviews/import-csv"
PIPELINE_KEY = "Luvey_AI_2026!"


def step1_scrape() -> int:
    print("\n-- Step 1: Playwright scrape (max 200 reviews) --")
...
def step2_clean() -> int:
    ...
    result = cleaner.clean_csv_path(
        ...
        report_path=BACKEND_DIR / "data" / "realme_p4_power_5g_clean_report.json",
    )
...
def step3_import_local() -> None:
    ...
    repo.upsert_product(
        ...
        product_name=PRODUCT_NAME,
        brand=BRAND,
        price_band="13000-18000",
        own_brand=False,
        parent_company="Realme",
...
def step4_post_to_render() -> None:
    ...
    print("  Waking Render...")
    try:
        health = requests.get("https://irip-api.onrender.com/health", timeout=40)
        print(f"  Health: {health.status_code}")
    except Exception as e:
        print(f"  Health check timed out (Render waking up): {e}")
    ...
    with open(CLEAN_CSV, "rb") as f:
        try:
            resp = requests.post(
                ...
                files={"file": ("realme_p4_power_clean.csv", f, "text/csv")},
                timeout=300,
            )
...
def main() -> None:
    print("IRIP - Realme P4 Power 5G: Scrape + Clean + Import")
    print(f"  DB    : {DB_PATH}")
